...
public sealed class EntityFactory : IEntityFactory
{
    ...
    //IAudioPlayer player)
    {
        ...
        //_lazer = assetProvider.Load<Sound>("Sounds/laser.sound");
        //_explosion = assetProvider.Load<Sound>("Sounds/asteroid-explosion.sound");
    ...
    //    var spriteName = _assetProvider.GetFiles("Sprites/PlayerShips").RandomPick();
    //    var sprite = _assetProvider.Load<Sprite>(spriteName);
    //    var laserSpriteName = _assetProvider.GetFiles("Sprites/Lasers").RandomPick();
    //    var laserSprite = _assetProvider.Load<Sprite>(laserSpriteName);

    //    var trailSpriteName = _assetProvider.GetFiles("Sprites/Trails").RandomPick();
    //    var trailSprite = _assetProvider.Load<Sprite>(trailSpriteName);
    //    var debris = _assetProvider.GetFiles("Sprites/Debris").Select(_assetProvider.Load<Sprite>).ToArray(); var reload = TimeSpan.FromMilliseconds(500);
    ...
    public Asteroid CreateAsteroid(AsteroidType type, Vector2 position, float direction)
    {
        ...
        var asteroid = new Asteroid(_draw, type, sprite, debri, velocity, new Vector2(GameRoot.Scale), rotationSpeed)
        {
            Position = position
        };
    ...
    //    var spriteName = _assetProvider.GetFiles("Sprites/Ufos").RandomPick();
    //    var sprite = _assetProvider.Load<Sprite>(spriteName);
    //    var blasterSpriteName = _assetProvider.GetFiles("Sprites/Blasters").RandomPick();
    //    var blasterSprite = _assetProvider.Load<Sprite>(blasterSpriteName);

    //    var debris = _assetProvider.GetFiles("Sprites/Debris").Select(_assetProvider.Load<Sprite>).ToArray();
